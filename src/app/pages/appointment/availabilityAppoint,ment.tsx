...
const AvailabilityAppointment = () => {
  ...
  const { appointmentId, adminSlotsList, appointmnetAvailability } =
  ...
  const getAdminAppointments = () => {
    ...
    let obj = {
      date: moment(dateState.date).format("YYYY-MM-DD"),
      data: { isBookedByAdmin: true, serviceTime: TimeState },
    };
  ...
  const fetchAppointmentAvailability: any = useCallback(() => {
    ...
    setTimeout(() =>{
      ...
      if (appointmnetAvailability && appointmnetAvailability.length > 0) {
        appointmnetAvailability.forEach((item) => {
          array.push({
            ...item,
                ...
                <FullCalendar
                  ...
                  datesSet={(dateInfo) => {
                    setFullCalendar(() => ({
                      ...
                      end: formatDateToISO(dateInfo.endStr),
                    }));
                    let obj: any = {
                      start: formatDateToISO(dateInfo.start),
                      end: formatDateToISO(dateInfo.endStr),
                    };
              ...
              <Container>
                ...
                <Row>
                  {adminSlotsList &&
                    ...
                    adminSlotsList.map((slot: any, index: number) => (
